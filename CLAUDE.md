...
- **Curadoria (relação com o Flamengo):** antes de gerar texto/publicar, uma chamada de LLM (`llm.generate`, batch — 1 chamada p/ todos) classifica cada vídeo como relacionado ou não, **julgando pelo CONTEÚDO** (não pela conta): o vídeo precisa falar do Flamengo/jogo/mercado, de um jogador/técnico ATUAL do elenco (lido de `article_writer/elenco_flamengo.json`) ou de um ÍDOLO/LENDA/ex-jogador histórico do Flamengo (Zico, Júnior, Adriano, etc.). Conta rubro-negra postando conteúdo genérico (workshop, promo) **não passa**. Os reprovados são descartados. `--no-curate` pula o filtro.
